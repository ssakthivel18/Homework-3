...
def init_p(l, w, seqs, nt_dict={"A":0, "C":1, "G":2, "T":3}):
    '''
...
def update_locations(l, w, p, seqs, nt_dict={"A":0, "C":1, "G":2, "T":3}):
    '''
    ...
    for i, sequence in enumerate(seqs):
        ...
        for j in range(l-w+1):
            ...
            for position, nt in enumerate(sequence[j:j+w]):
...
def update_profile(l, w, z, seqs, nt_dict={"A":0, "C":1, "G":2, "T":3}):
    '''
    ...
    for nt in nt_dict.keys():
        ...
        for i, sequence in enumerate(seqs):
            for j in range(l-w+1):
                if sequence[j] == nt:
                    sum_z += z[i][j]
        n[nt_dict[nt]][0] = sum_z

    # Normalize p and return
    for k in range(w+1):
        for nt in nt_dict.keys():
            p[nt_dict[nt]][k] = (n[nt_dict[nt]][k]) / (np.sum(n[:,k])+4)
...
def run_EM(w, seqs, nt_dict={"A":0, "C":1, "G":2, "T":3}, epsilon=2**-64):
    '''
    ...
    #initialize p
    p_prev = init_p(l, w, seqs, nt_dict={"A":0, "C":1, "G":2, "T":3})

    while not no_change:
        #update z_t
        z_t = update_locations(l, w, p_prev, seqs, nt_dict={"A":0, "C":1, "G":2, "T":3})
        
        #update p_t
        p_t = update_profile(l, w, z_t, seqs, nt_dict={"A":0, "C":1, "G":2, "T":3})

        #stop loop if the difference between p_t and p_prev is small enough 
        if np.abs(p_t - p_prev).max() < epsilon:
            no_change = True
...
if __name__ == '__main__':
    ...
    p_final, z_final = run_EM(width, sequences, nt_dict={"A":0, "C":1, "G":2, "T":3}, epsilon=0.0001)
    print('\nFinal p:')
    print(p_final)
