...
Original file is located at
    https://colab.research.google.com/drive/1686REJAORuB1Jn-90Ny1i7jGnvyVV5Hm
"""
...
def init_p(l, w, seqs, nt_dict):
    ...
    '''
    p_0 = np.zeros((4, w+1))
    # set a uniform background probability for each nucleotide. basically this is 0.25
    for i in nt_dict.keys():
        p_0[nt_dict[i]][0] = 0.25    #... #TODO
    # count nucleotides in all possible motifs for each sequence
    for sequence in seqs:
        # i in range(l-w+1) -> all possible motif starting positions
        for i in range(l-w+1):
            ####sequence_section = sequence[i:w] #grab the 8 length motif we want each time
            # j in range(w) -> position within the motif
            for j in range(w):
                # iterating thru each individual part of the motif
                # Fill in p_0          #... #TODO
                motif_position = j+1  # need to add one because we already have the background probability of the actual j term equals 0.25
                if sequence[j+i] == "A": # if you do only j, then it incorrectly doesn't consider that the motif also moves. so have to do j+i
                    p_0[nt_dict["A"]][motif_position] += 1 # increase tally by adding 1
                elif sequence[j+i] == "C":
                    p_0[nt_dict["C"]][motif_position] += 1
                elif sequence[j+i] == "G":
                    p_0[nt_dict["G"]][motif_position] += 1
                else:
                    p_0[nt_dict["T"]][motif_position] += 1
...
def update_locations(l, w, p, seqs, nt_dict):
    ...
    update locations: This function updates Z, a matrix representing the probability of the motif starting
at each location in each sequence, based on the previous estimate of p, the motif profile.
NOTE: scale each probability by some constant value (10 works well in this case). Since we normalize Z so that
the sum of each row is equal to 1
    ...
    for i, sequence in enumerate(seqs):
        #consider each possible starting position for the motif j. j goes from 0 till the last possible motif start position
        for j in range(l-w+1):
            # initialize a list of conditional probabilities
            p_vals = []
            #consider the nucleotide at each position in the sequence
            for position, nt in enumerate(sequence):
                # HINT: what is the probability of seeing nt at this position,given that the motif begins at position j?
                if position >= j and position < (j+w): # check if position of sequence is in the motif including j up to j+w
                    # if position is 7 and j is 2, check that 7 is between 2 and 10 motif. the want prob that nt is in position 7 of seq
                    # which is actually the fifth pos in motif and 6th column of p. 7-2 = 5 + 1 = 6 since first column p is background
                    prob = 10 * p[nt_dict[nt]][position - j + 1] # if it is, get the probability that nt is in the pos-j+1 position of motif
                else:
                    prob = 10 * p[nt_dict[nt]][0] # if not, this is the background prob, first/0th column of p matrix
                p_vals.append(prob)
            # HINT: multiply each probability by 10 to avoid underflow errors (see pdf)
            #p_vals = 10 * p_vals
            # update Z_ij #... #TODO
            conditional_prob = np.prod(p_vals)
            z[i][j] = conditional_prob
    # Normalize z so that the sum of each row is equal to 1
...
def update_locations_E_or_M():
    ...
    '''
    # we are doing expectation step of algorithm here
            ...
            for i, seq in enumerate(seqs):
                ...
                for y in range(l - w + 1):
                    ...
                    if seq[y+k-1] == nt:
                        probb.append(z[i][y])
                # update n
                        n[nt_dict[nt]][k] = sum(probb)

    # fill in n for k == 0
    # background is all the times nt appears in sequence minus all the times nt appears in motif
    for j in range(1, w + 1):
        for nt in nt_dict.keys():
            for i, seq in enumerate(seqs):
                total_nt = seq.count(nt)
                appear_motif = 0
                for m in range(l - w + 1):
                    if seq[m+j-1] == nt:
                        appear_motif += 1
                        n[nt_dict[nt]][j] = total_nt - appear_motif
...
def update_profile_E_or_M():
    ...
    Have this function return 'E' for expectation or 'M' for maximization.
    '''
    # we are doing maximization step of algorithm here
    return 'M' # or return 'E'  # TODO
...
def run_EM(w, seqs, nt_dict, epsilon=2**-64):
    ...
    Run the EM algorithm for motif-finding by putting together the other functions that you
    implemented: init p, update locations, and update PROFILE. Terminate the algorithm once the estimate
    of p effectively stops changing (the difference between the current p and the previous p is less than
    epsilon for each value in p).

    Hint: Use the previously defined functions init_p, update_locations, and update_motif THIS IS WRONG. UPDATE_PROFILE
    ...
    run EM: Run the EM algorithm for motif-finding by putting together the other functions that you
implemented: init p, update locations, and update motif. Terminate the algorithm once the estimate
of p effectively stops changing (the difference between the current p and the previous p is less than
epsilon for each value in p).
    '''
    ...
    while not no_change:
        ...
        if np.all(abs(p_t - p_prev) < epsilon):
            no_change = True
        else:
            # update p_prev
            p_prev = p_t
...
if __name__ == '__main__':
    ...
    # Note - if the rows of z don't add up to 1, you might be encountering a underflow error
