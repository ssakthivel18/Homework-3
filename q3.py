...
Original file is located at
    https://colab.research.google.com/drive/13tn0NH1JpBI0WlQtAi25LGcvVegUQJUM
"""
...
def read_data(file_name, test_size=0.2):
    ...
    data = pd.read_csv(file_name, header=None, sep=' ', names=['sequences', 'labels'])
    sequence = data['sequences'].values
    label = data['labels'].values

    # Shuffle and split the data
    X_train, X_test, y_train, y_test = train_test_split(sequence, label, shuffle=True, test_size=test_size)
...
def build_transition_matrix(data, k):
    ...
    kmers = [''.join(x) for x in product(['A', 'C', 'T', 'G'], repeat=k)]
    trans_prob = np.zeros((len(kmers), len(kmers)))
    # generate a list of all possible kmers combinations, use itertools.product() - read the docs
    ...
    kmer2idx = {kmer: i for i, kmer in enumerate(kmers)}   # kmer2idx['ATC'] = 4, for example
    ...
    for seq in data:
        for i in range(len(seq) - k):
            ...
            next_nt = seq[i+k]
            trans_prob[kmer2idx[current_kmer]][nt2idx[next_nt]] += 1

    trans_prob += 1 # apply 1 pseudocount
...
def log_odds_ratio(seq, k, pos_kmer2idx, pos_trans_probs, neg_kmer2idx, neg_trans_probs):
    ...
    nt2idx = {'A': 0, 'G': 1, 'T': 2, 'C': 3, '':4}
    score = 0
    for i in range(len(seq) - k):
        # calculate the log odds ratio using the variables passed
        kmer = seq[i:i+k]
        if kmer not in pos_kmer2idx or kmer not in neg_kmer2idx:
            continue
        else:
          pos_kmer_idx = pos_kmer2idx[kmer]
          neg_kmer_idx = neg_kmer2idx[kmer]
          pos_prob = pos_trans_probs[pos_kmer_idx, nt2idx[seq[i+k]]]
          neg_prob = neg_trans_probs[neg_kmer_idx, nt2idx[seq[i+k]]]
          score += np.log(pos_prob / neg_prob)
    return score
...
def classify(seq, k, pos_kmer2idx, pos_trans_probs, neg_kmer2idx, neg_trans_probs):
    ...
    # TODO
    log_odds = log_odds_ratio(seq, k, pos_kmer2idx, pos_trans_probs, neg_kmer2idx, neg_trans_probs)
    if log_odds > 0:
        return 1
    else:
        return 0
